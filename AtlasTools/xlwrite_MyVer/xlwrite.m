...
%   20141205 - RB: Added autoloading of JAVA-classes as suggested by "J"
%
...
if((exist('org.apache.poi.ss.usermodel.WorkbookFactory', 'class') ~= 8) || (exist('org.apache.poi.hssf.usermodel.HSSFWorkbook', 'class') ~= 8) || (exist('org.apache.poi.xssf.usermodel.XSSFWorkbook', 'class') ~= 8))
    try% RB-change according to "J" from MATLABcentral
        cpath=fileparts(which(mfilename('fullpath')));
...
status = 1;
if(status)
    message = 'Execution of xlwrite-"MyVer" was successful.';
else
    message = 'error';
end
