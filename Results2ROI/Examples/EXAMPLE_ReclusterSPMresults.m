%% get test data
try
    load('EXAMPLEdata_ReclusterSPMresults.mat');
catch
    BaseDir = fileparts(mfilename('fullpath'));
    load([BaseDir,filesep,'EXAMPLEdata_ReclusterSPMresults.mat']);
end

%% assign data
Coords    = xSPM.XYZmm'; %dimensions are columns and datapoints are rows
StatsVals = xSPM.Z'; %statistic values per voxel
VoxCoords = xSPM.XYZ';

%% ZYXresort?
ChoiceUseZYXresort = questdlg('Use ZYXresort on the voxel locations?','Use ZYXresort?','ZYXresort','unsorted','ZYXresort');
switch(ChoiceUseZYXresort)
    case 'ZYXresort'
        UseZYXresort = 1;
        ResortIndices = ZYXresort(Coords);
        Coords    = Coords(ResortIndices,:);
        StatsVals = StatsVals(ResortIndices);
        VoxCoords = VoxCoords(ResortIndices,:);
    otherwise
        ChoiceUseZYXresort = 'unsorted';
end
...
if(strcmp('Yes',questdlg('Use Connections matrix?','ConnectMat?','Yes','No','No')))
    ...
    figure(); clf;
    subplot(1,3,1); imagesc(ConnectionsMat,[0 1]); colormap(gray); title(['connectivity matrix. [',ChoiceUseZYXresort,']']);
...
[AX,H1,H2]=plotyy(1:length(LocMaxStruct.Iterations.NClusters),LocMaxStruct.Iterations.NClusters,1:length(LocMaxStruct.Iterations.DistanceToConverge),LocMaxStruct.Iterations.DistanceToConverge./max(LocMaxStruct.Iterations.DistanceToConverge(:)));
set(H1,'LineStyle','-','Marker','x');
set(H2,'LineStyle','-','Marker','o');
xlabel('Iteration'); title(['Convergence & number of Clusters for LocMax search [Minimum separation ',num2str(LocMaxMinDist_mm),'mm; (',ChoiceUseZYXresort,')]']);
...
subplot(2,3,1); imagesc(Dist_ORG); title(['Original distances [',ChoiceUseZYXresort,']']); %axis('square');
...
if(strcmp('Yes',questdlg('Create NIFTI from Clustering and then display Clustering?','Display Clusters?','Yes','No','Yes')))
    if(~exist('ConnectionsMat','var'))
        ShowOverlayClusters(LocMaxStruct.ClusterNo,VoxCoords,xSPM.Vspm,[pwd,filesep,'Clusters_',num2str(LocMaxMinDist_mm),'mm_MinDist_',date]);
    else
        ShowOverlayClusters(LocMaxStruct.ClusterNo,VoxCoords,xSPM.Vspm,[pwd,filesep,'Clusters_',num2str(LocMaxMinDist_mm),'mm_MinDist_UsingConnectionsMatrix_',date]);
    end
end
